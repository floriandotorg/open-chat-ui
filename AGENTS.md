...
**iOS PWA standalone gotcha.** When the app runs as an installed PWA on iOS (`display-mode: standalone`), WebKit fails to paint `backdrop-filter` on elements that are `position: absolute|fixed` inside an `overflow: hidden` ancestor — which is every glass bar in this app. The surface degrades to its flat semi-transparent fill while still rendering correctly in Chrome and Mac Safari. `layout.css` applies `transform: translateZ(0)` to the three glass classes unconditionally to force GPU layer promotion so the filter has a source to sample. This is safe with the Tailwind v4 utilities used for the mobile sidebar slide because Tailwind v4 maps `translate-x-*` to the CSS `translate` property (not `transform`), and `transition-transform` transitions both. Do NOT replace the explicit transform with `will-change: backdrop-filter` — the hint isn't strong enough to fix iOS PWA standalone. If you add a new glass utility, include it in that block.
