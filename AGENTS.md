...
**iOS PWA standalone gotcha.** When the app runs as an installed PWA on iOS (`display-mode: standalone`), WebKit fails to paint `backdrop-filter` on elements that are `position: absolute|fixed` inside an `overflow: hidden` ancestor — which is every glass bar in this app. The surface degrades to its flat semi-transparent fill while still rendering correctly in mobile Safari and Mac Safari. `layout.css` has a scoped `@supports (-webkit-touch-callout: none) { @media (display-mode: standalone) }` block that adds `will-change: backdrop-filter` to the three glass classes to force layer promotion. Do NOT swap that for `transform: translateZ(0)` — it would clobber the Tailwind translate utilities used for the mobile sidebar slide animation. If you add a new glass utility, include it in that block.
